...
void InventoryItem::GetItemStatusRow( PyPackedRow* into ) const
{
    ...
    into->SetField( "charge",        new PyFloat( (mAttributeMap.HasAttribute(AttrCharge) ? (bool)GetAttribute(AttrCharge).get_float() : 0) ) );
    ...
    into->SetField( "incapacitated", new PyBool( (mAttributeMap.HasAttribute(AttrIsIncapacitated) ? (bool)GetAttribute(AttrIsIncapacitated).get_int() : 0) ) );
}
